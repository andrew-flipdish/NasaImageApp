...
class ApiCaller {
    ...
    func callAPI(completion: @escaping ((NasaResponse?) -> Void)){
        guard let url = URL(string: "\(api)\(apiKey)") else {
            print("Invalid URL")
            completion(nil)
            return
        }
    ...
    func callAPIDate(startDate: String, completion: @escaping (([NasaResponse]?) -> Void)){
